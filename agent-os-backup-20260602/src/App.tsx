...
const DEFAULT_MODELS = [
  { id: "openrouter/free", name: "OpenRouter Free (Random) ⚡", ctx: "200K", type: "free", prompt: "0", provider: "openrouter" },
];
    ...
    <div className="space-y-2">
      ...
      <div className="mt-3 pt-3 border-t border-white/[0.04]">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[9px] text-gray-500 font-bold uppercase tracking-wider select-none flex items-center gap-1.5">
            <Radio size={10} className="text-blue-400" /> Google Direct API
          </span>
          <button 
            onClick={async () => {
              try {
            ...
            className="text-[8px] px-1.5 py-0.5 bg-rose-500/10 text-rose-400 rounded hover:bg-rose-500/20 transition-colors"
            title="Reset Gemini Cooldowns"
          >
            Reset Quota
          </button>
        </div>
        <select 
          className="w-full bg-[#1e1e24] border border-white/[0.05] rounded-md text-[10px] text-gray-300 p-1.5 outline-none focus:border-blue-500/50"
          onChange={async (e) => {
            try {
              await fetch('/api/set-provider', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ modelId: e.target.value }) });
            } catch (_) {}
          }}
        >
          ...
          <option value="google/gemini-3.5-flash">Gemini 3.5 Flash (Puter)</option>
        </select>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Dynamic models from server (moved inside component — hooks must be inside components)
  ...
  // Merge: server-discovered models take priority, plus hardcoded defaults
  const MODELS = (() => {
    if (serverModels.length > 0) {
      const seen = new Set();
      const merged: any[] = [];
      for (const m of [...serverModels, ...DEFAULT_MODELS]) {
    ...
    return DEFAULT_MODELS.map(m => ({ ...m, selected: false, tier: 'free' }));
  })();
                          ...
                          >
                            <div className="min-w-0 flex-1">
                              <div className="text-xs font-semibold truncate flex items-center gap-1.5">
                                ...
                                {model.tier === 'cheap' && (
                                  <span className="text-[7px] px-1 py-0.5 rounded bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 font-bold">¢</span>
                                )}
                                {model.provider === 'nous' && (
                                  <span className="text-[7px] px-1 py-0.5 rounded bg-purple-500/10 border border-purple-500/20 text-purple-400 font-bold">NOUS</span>
                                )}
                              </div>
